...
for k = 1 : data_length
    ...
    % Phase estimation    
    % TODO 2:
    theta_estimator = 1 / 4 * angle(- payload_data(k)^4);
    possible_theta = theta_estimator + [0:pi/2:2*pi];
    [~,index] = min(abs(possible_theta - theta_hat_no_filter(k)));
    theta_hat_no_filter(k+1) = wrapTo2Pi(possible_theta(index));
    % TODO 3:
    payload_data_no_filter(k) = payload_data(k) * exp(-1j * theta_hat_no_filter(k));
    % TODO 4
    theta_estimator = 1 / 4 * angle(- payload_data(k)^4);
    possible_theta = theta_estimator + [0:pi/2:2*pi];
    [~,index] = min(abs(possible_theta - theta_hat(k)));
    tmp = wrapTo2Pi(possible_theta(index));
    theta_hat(k+1) = 0.01 * tmp + 0.99 * theta_hat(k);
    payload_data(k) = payload_data(k) * exp(-1j * theta_hat(k));
